...
async def login(client,message,playerlist,gamestate,rules,roles):	
	def pred(m): return m.channel == message.channel

	#Login Phase
	gamestate[0] = 1
	await message.channel.send(loginStr)
	while gamestate[0] == 1:
		reply = await client.wait_for('message', check=pred)
...
async def night(client,message,playerlist,gamestate,rules,roles,canreject,cantreject):
	...
	for key in roles:
		...
		if key == "Galahad, Loyal Servant of Arthur" or key == "Tristan, Loyal Servant of Arthur" or key == "Guinevere, Loyal Servant of Arthur" or key == "Lamorak, Loyal Servant of Arthur":
			...
			await message.channel.send(roles[key],loyalDM.format(roles[key].name,key))	
		if key == "Agravain, Minion of Mordred":
			canreject.append(roles[key])
			await message.channel.send(roles[key],minionDM.format(roles[key].name,key,toString(evillist)))
		if key == "Merlin":
			cantreject.append(roles[key])
			await message.channel.send(roles[key],merlinDM.format(roles[key].name,key,toString(merlinlist)))
		if key == "The Assassin":
			canreject.append(roles[key])
			await message.channel.send(roles[key],assassinDM.format(roles[key].name,key,toString(evillist)))
		if key == "Mordred":
			canreject.append(roles[key])
			await message.channel.send(roles[key],mordredDM.format(roles[key].name,key,toString(evillist)))
		if key == "Morgana":
			canreject.append(roles[key])
			await message.channel.send(roles[key],morganaDM.format(roles[key].name,key,toString(evillist)))
		if key == "Percival":
			cantreject.append(roles[key])
			await message.channel.send(roles[key],percivalDM.format(roles[key].name,key,toString(percivallist)))
...
async def quest(client,message,playerlist,gamestate,rules,roles,boardstate,names):
	...
	while gamestate[0] == 3:
		votetrigger = await client.wait_for('message', check=pred)
			...
			if valid == 1:
				if len(names) == len(set(names)):
					if len(names)-1 == rules[(gamestate[2]-1)]:
						...
						await message.channel.send(malformedStr.format(rules[gamestate[2]-1]))
				else:
					await message.channel.send(duplicateStr.format(rules[gamestate[2]-1]))
...
async def teamvote(client,message,playerlist,gamestate,rules,roles,boardstate,names):
	...
	while gamestate[0] == 4:
		...
		for j in range(0,len(playerlist)-1):
			vc += 1
			pmtrigger = await client.wait_for(check=votecheck)
...
async def privatevote(client,message,playerlist,gamestate,rules,roles,boardstate,names,canreject):
	while gamestate[0] == 5:
		...
		for j in range(0,votecount):
			pmtrigger = await client.wait_for(check=privatevotecheck)
...
async def gameover(client,message,playerlist,gamestate,rules,roles,boardstate,names,canreject,cantreject):
	...
	if gamestate[4] == 3:
		...
		ass = await client.wait_for(check=assassincheck)
	...
	roleStr = "\n"
	for key in roles:
		#await message.channel.send(str(roles[key])+" is "+str(key))
...
async def addscore(client, message, user):
	...
	if user.id in score:
		...
		score[user.id] = 30
	#await message.channel.send(str(user.name)+" now has "+str(score[user.id])+" dollarydoos")
	score.sync()
	score.close()
